import styles from "./IconCart.module.css";

const IconCart = ({ isActive }: { isActive?: boolean }) => {
	const getRectClass = () => {
		return isActive ? styles.rectActive : styles.rectInactive;
	};

	const getPathClass = () => {
		return isActive ? styles.pathActive : styles.pathInactive;
	};

	return (
		<svg
			viewBox="0 0 24 24"
			width="24.000000"
			height="24.000000"
			fill="none"
			className={styles.icon}
		>
			<rect
				id="feather-icon / shopping-cart"
				width="24.000000"
				height="24.000000"
				x="0.000000"
				y="0.000000"
				className={`${styles.rect} ${getRectClass()}`}
			/>
			<path
				id="Oval (Stroke)"
				d="M5.5 21C5.5 19.6193 6.61929 18.5 8 18.5C9.38071 18.5 10.5 19.6193 10.5 21C10.5 22.3807 9.38071 23.5 8 23.5C6.61929 23.5 5.5 22.3807 5.5 21ZM8 19.5C7.17157 19.5 6.5 20.1716 6.5 21C6.5 21.8284 7.17157 22.5 8 22.5C8.82843 22.5 9.5 21.8284 9.5 21C9.5 20.1716 8.82843 19.5 8 19.5Z"
				className={getPathClass()}
			/>
			<path
				id="Oval (Stroke)"
				d="M17.5 21C17.5 19.6193 18.6193 18.5 20 18.5C21.3807 18.5 22.5 19.6193 22.5 21C22.5 22.3807 21.3807 23.5 20 23.5C18.6193 23.5 17.5 22.3807 17.5 21ZM20 19.5C19.1716 19.5 18.5 20.1716 18.5 21C18.5 21.8284 19.1716 22.5 20 22.5C20.8284 22.5 21.5 21.8284 21.5 21C21.5 20.1716 20.8284 19.5 20 19.5Z"
				className={getPathClass()}
			/>
			<path
				id="Union"
				d="M1 0.5C0.723858 0.5 0.5 0.723858 0.5 1C0.5 1.27614 0.723858 1.5 1 1.5L4.58051 1.5C5.33783 5.79147 6.10333 10.0643 6.74931 14.3709C6.93288 15.5947 7.98416 16.5 9.22165 16.5L19.3597 16.5C20.5514 16.5 21.5774 15.6588 21.8111 14.4903L23.2503 7.29417C23.436 6.36599 22.726 5.5 21.7795 5.5L7.16046 5.5C6.8575 5.5 6.57797 5.58901 6.34436 5.74093L5.49239 0.913107C5.45023 0.674179 5.24262 0.5 5 0.5L1 0.5ZM6.666 7.07417C6.62065 6.77187 6.85478 6.5 7.16046 6.5L21.7795 6.5C22.095 6.5 22.3316 6.78866 22.2698 7.09806L20.8305 14.2942C20.6903 14.9953 20.0747 15.5 19.3597 15.5L9.22165 15.5C8.47916 15.5 7.84839 14.9568 7.73825 14.2225L6.666 7.07417Z"
				className={getPathClass()}
			/>
		</svg>
	);
};

export default IconCart;
